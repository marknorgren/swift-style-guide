Swift Style Guide and General Architecture patterns guidelines.

Document initially forked from [Linkedin's Swift Style Guide](https://github.com/linkedin/swift-style-guide)
...
- **1.1** Spaces not tabs
- **1.2** 4 spaces for tab (using tabs as spaces). 4 spaces for tabs is Xcode default.
- **1.3** Avoid long lines - hard maximum of 160 characters (Xcode->Preferences->Text Editing->Page guide at column: 160 is helpful for this)
- **1.4** Do not place opening braces on new lines - use the [1TBS style](https://en.m.wikipedia.org/wiki/Indent_style#Variant:_1TBS). 
- **1.5** Follow Xcode's recommended indentation style (i.e. your code should not change if CTRL-I is pressed). When declaring a function that spans multiple lines, prefer using that syntax to which Xcode, as of version 8.2, defaults.
...
- **1.6** When calling a function that has many parameters, put each argument on a separate line with a single extra indentation.
...
- **1.7** When dealing with an implicit array or dictionary large enough to warrant splitting it into multiple lines, treat the `[` and `]` as if they were braces in a method, `if` statement, etc. Closures in a method should be treated similarly.
...
- **1.8** Prefer using local constants or other mitigation techniques to avoid multi-line predicates where possible.
...
- **2.1** There is no need for Objective-C style prefixing in Swift (e.g. use just `QuoteViewController` instead of `DTNQuoteViewController`).
...
- **2.4** "Excessive use of made up acronyms is a significant impediment to communication" -[Elon Musk on acronyms](https://gist.github.com/mrkd/cb9a2f1113e18af1c8dd#on-acronyms). Do not use acronyms whenever possible and **DO NOT** create new acronyms.
- **2.4** When dealing with an acronym or other name that is usually written in all caps, use standard english to write the name out in all lowercase. If name is a type name (e.g. `struct`, `enum`, `class`, `typedef`, `associatedtype`, etc.). title case all.

```
// If ApiIcloudController is a type
API iCloud controller --> api icloud controller --> api Icloud Controller --> ApiIcloudController

// If apiIcloudController is an instance
api iCloud controller --> api icloud controller --> api Icloud Controller --> apiIcloudController
```
...
- **2.7** Include type information in constant or variable names when it is not obvious otherwise.
...
- **3.1.3** Be wary of retain cycles when creating delegates/protocols for your classes; typically, these properties should be declared `weak`.


- **3.1.4** Be careful when calling `self` directly from an escaping closure as this can cause a retain cycle - use a [capture list](https://developer.apple.com/library/ios/documentation/swift/conceptual/Swift_Programming_Language/Closures.html#//apple_ref/doc/uid/TP40014097-CH11-XID_163) when this might be the case:
...
- **3.1.5** Avoid writing out an `enum` type where possible - use shorthand.
...
- **3.1.6** **REVIEW** Don’t use shorthand for class methods since it is generally more difficult to infer the context from class methods as opposed to `enum`s. //
...
- **3.1.7** **REVIEW** Don't place parentheses around control flow predicates.
...
- **3.1.8** Prefer not writing `self.` unless it is required.


- **3.1.9** **REVIEW** When writing methods, keep in mind whether the method is intended to be overridden or not. If not, mark it as `final`, though keep in mind that this will prevent the method from being overwritten for testing purposes. In general, `final` methods result in improved compilation times, so it is good to use this when applicable. Be particularly careful, however, when applying the `final` keyword in a library since it is non-trivial to change something to be non-`final` in a library as opposed to have changing something to be non-`final` in your local project.


- **3.1.10** When using a statement such as `else`, `catch`, etc. that follows a block, put this keyword on the same line as the block. Again, we are following the [1TBS style](https://en.m.wikipedia.org/wiki/Indent_style#Variant:_1TBS) here. Example `if`/`else` and `do`/`catch` code is below.
...
- **3.1.11** **REVIEW** Prefer `static` to `class` when declaring a function or property that is associated with a class as opposed to an instance of that class. Only use `class` if you specifically need the functionality of overriding that function or property in a subclass, though consider using a `protocol` to achieve this instead.
...
- **3.1.13** For the purpose of namespacing a set of `static` functions and/or `static` properties, prefer using a caseless `enum` over a `class` or a `struct`. This way, you don't have to add a `private init() { }` to the container.
...
- **3.2.3** **REVIEW** In general, do not write the `internal` access modifier keyword since it is the default.

Should this been done so it explicitly shows you thought about the access control instead of forgetting about it?
...
via: http://stackoverflow.com/a/39027497
...
- **4.1** Following MVVM pattern.
- **4.2** Avoid logic in Views. Views should update when passed a View Model.
...
- https://www.raywenderlich.com/133102/swift-style-guide-april-2016-update
